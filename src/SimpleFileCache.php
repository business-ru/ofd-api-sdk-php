...
use Ofd\Api\Exception\SimpleFileCacheException;
use Ofd\Api\Exception\SimpleFileCacheInvalidArgumentException;
...
class SimpleFileCache implements CacheInterface
{
	...
	{
		if (!is_dir($this->cachePath) && !mkdir($concurrentDirectory = $this->cachePath) && !is_dir(
				$concurrentDirectory
			)) {
	...
	{
		if (!is_string($key)) {
			throw new SimpleFileCacheInvalidArgumentException('Ключ должен быть строкой');
		}
